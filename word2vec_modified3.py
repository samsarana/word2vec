...
def train_batch_sg(model, sentences, alpha, work=None):
    ...
    for sentence in sentences:
        ...
        for pos, word in enumerate(word_vocabs):
            ...
            stop = min(pos + model.window + 1 - reduced_window, len(sentence))
            for pos2, word2 in enumerate(word_vocabs[start:stop], start):
                if pos2 != pos and model.index2word[word2.index][0].isupper(): # only trains w props (indicated by uppercase first letter) as inputs, not operators (strings representing laws all begin with lowercase letters)
                    #print('Training with {} as input and {} as target'.format(model.index2word[word2.index], model.index2word[word.index]))
